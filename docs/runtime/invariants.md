...
2. Run **MUST** 只收敛到 `completed`、`failed` 或 `cancelled` 中的一个终态；内存状态与 `RunStore` **MUST** 使用一次性终态 CAS。
...
5. 终态后的控制事件、模型结果和工具结果 **MUST NOT** 改变 Context、事件流或终态；拒绝行为 **SHOULD** 可观测。
...
4. 持久 Event Log **SHOULD** 使用至少一次投递；消费者 **MUST** 依据稳定 ID 幂等处理重复事件。
5. Event Log **MUST** 只追加，修正通过新事件表达。
...
5. 稳定失败原因 **MUST** 包括 `wall_time_exceeded`、`idle_timeout`、`decision_budget_exhausted`、`token_budget_exhausted`、`no_progress`、`policy_error`、`context_conflict`、`adapter_timeout`、`runtime_shutdown` 和 `internal_error`。
