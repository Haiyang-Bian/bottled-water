...
本记录阶段未实现正式持久化修复、跨 CLI 排他、完整依赖复核和外部移动竞态处理。后续的依赖
复核及 Job 子集进展单独记录于[有限原生放行清单](standing-permissions-completion-0.2.3.md)。
测试树包含未授权 Private，
...
固定初始化后的完整软件正负向矩阵已经完成。本记录之后补充的策略隔离、依赖异常和嵌套 Job
证据见[有限原生放行清单](standing-permissions-completion-0.2.3.md)，不改变此处旧启动结构的历史结果。
生产文件 worker、Runtime 组装、持久化管理与完成事务仍未接入。
