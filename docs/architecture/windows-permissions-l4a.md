# AgentHub 0.2.3 目标：L4a Windows 细粒度权限与受限执行

更新：2026-09-15。**当前仅完成 P0 和 P1 原生实验，P1 尚未通过，不发行 0.2.3。**
...
不能把命令未启动、程序报错或模型愿意配合计为权限通过。当前 Git 操作失败，因此 P2–P4 未启动。
...
| P1 原生边界 | 实验及失败证据已保存；Git 未通过，暂停后续发行推进 |
...
下一步首先完善并验证**系统对象初始化**，仍保留 LPAC 路线：

1. 为 Git 路径转换列出精确的 NT 对象、必要访问掩码和依赖能力 SID，与业务目录 SID 分离。
2. 核实对象权限的修改条件、最小权限、并发补偿、清理归属及重启后的有效性。
3. 如需开机初始化组件或其他新生命周期机制，先补充安装/卸载及保护设计，再评审其范围；
   本轮实验未创建服务、启动项或计划任务，也未修改 NT 系统对象 ACL。
4. 在保持 A 只读、B 可修改、C 不可读和禁网的条件下通过真实 Git show/config/diff。
5. 补齐并行身份、句柄泄漏、宿主进程访问、硬链接/junction/路径替换、超时和强制退出等 P1 测试。
