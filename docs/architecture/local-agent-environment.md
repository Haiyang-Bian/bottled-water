...
> 状态：0.2.0 已实现 L1；0.2.1 已实现 [L2 基础记忆](./foundational-memory-l2.md)，实测边界见[验收记录](../acceptance/foundational-memory-0.2.1.md)。L3–L4 待实现。原有 [0.2.0 验收记录](../acceptance/local-environment-0.2.0.md)保留。
...
下表保留 0.1.7 的历史基线对照。L1 中的身份、会话恢复与位置已在 0.2.0 实现，L2 基础记忆已在 0.2.1 实现；资源知识库及强隔离仍是目标，当前命令以 [CLI 说明](../cli.md)为准。
...
全局恢复与工作位置由 0.2.0 固化；0.2.1 增加 `/memory` 与 `agenthub memory` 管理入口及三个模型工具。已实现命令以 [CLI 文档](../cli.md)为准，下面涉及资源知识库和多助手的入口仍属于后续设计。
