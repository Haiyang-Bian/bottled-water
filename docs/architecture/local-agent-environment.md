...
> 当前状态：L1–L3 已在 0.2.0–0.2.2 实现；0.2.3 原生使用闭环已验收，普通 CLI 取消逐目录 trust，使用当前普通用户可访问的文件与网络，详见[当前验收](../acceptance/native-user-experience-0.2.3.md)。L4 强隔离暂缓，多助手共享待实现；旧验收记录保留原时点结论。
> 最初设计基线：`d43f07ba954b74ea544815f4ba8a294766109f34`，共享包 `0.1.7`。下文细粒度授权属于设计方向，不作为 0.2.3 普通 CLI 已提供的限制；当前操作以 [CLI 手册](../cli.md)为准。
...
下表保留 0.1.7 的历史基线对照。L1 身份与位置、L2 基础记忆、L3 资源目录已分别在 0.2.0–0.2.2 实现；强隔离仍是目标，当前命令以 [CLI 说明](../cli.md)为准。
...
0.2.1 固定采用用户明确管理保存立即生效、模型仅提候选的规则；普通对话里的“记住”也须用户采纳后生效。任务结束提示候选数量，不自动弹审批。记忆复用许可独立于文件许可。私有推理和凭据不进入可召回投影，外部文本不提升为系统规则。
...
全局恢复与工作位置由 0.2.0 固化；0.2.1 增加 `/memory`，0.2.2 增加 `/resources`、`/software` 和任务查询。已实现命令以 [CLI 文档](../cli.md)和[资源说明](../resources.md)为准，下面涉及多助手的入口仍属于后续设计。
...
| `agent_subsystems/memory` / `context` | 记忆规则、检索、候选校验和复核 / 上下文装配与实际使用清单 | 跨 scope 隐式拼接原始历史 |
