...
契约按领域归属维护，不建立容纳全部服务的 `common` 大包。`agent_runtime` 目前也不是纯 Kernel：其中的 AgentLoop、工具和 Workflow 模块需要按职责重新归属。详情见[模块目录](./subsystems.md)。
...
以下名称是设计用语，尚非可导入的已发布 API。实现时先检查现有接口能否增量演进，避免并存两套同义类型。
