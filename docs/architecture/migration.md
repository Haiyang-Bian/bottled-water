...
> 当前补充：L1–L3 已分别在 0.2.0–0.2.2 实现，0.2.3 原生跨目录/软件/网络闭环已验收。下述 M0–M4 表保留早期迁移范围，不是当前所有版本的完成清单；最新状态、产物和暂停的 L4 见[归档交接](../operations/archive-handoff-0.2.3.md)，方向与剩余拆分见[开发阶段](./local-agent-roadmap.md)。
...
## 10. 从目录级 CLI 到本机持续记忆（L1 已实现）

历史基线为 `d43f07b` / `0.1.7`。其中会话恢复的目录硬检查、cwd 与 grants 共用 root 已在 0.2.0 L1 拆分；scope 内的 AgentMemory 继续保留，长期记忆待 L2。具体步骤见 [L1 实施计划](./local-environment-l1.md)，实测见[验收](../acceptance/local-environment-0.2.0.md)。

| 迁移对象 | 保留 | 增量改变 | 阶段 |
| --- | --- | --- | --- |
| 会话 | session/Run ID、历史、锁、活动排序、失败恢复 | 环境/拥有者归属、全局候选与显式目录筛选 | L1 |
| 执行位置 | 真实路径校验、命令 cwd 记录、取消与租约 | 保存/切换位置，独立 grants，每调用位置快照 | L1 |
| 任务上下文 | Scope CAS、预算、工具配对、续接游标 | 增加长期记忆 contributor 和使用修订清单 | L2 |
| 记忆 | Journal 来源事实、脱敏与既有数据 | 独立记录/修订/遗忘/处理任务，跨 scope 授权检索 | L2 |
| 资源 | 文件/进程/Git 驱动 | 资源与软件目录、别名、位置再验证 | L3 |
| 共享与权限 | 授权入口、驱动能力声明、唯一终态 | 多助手共享与并发修订，细粒度动作和 Windows 强制执行 | L4 |

迁移不得把所有任务合成一个 Context scope，不得把旧全部日志自动提升为共享记忆，也不得因移除目录限制而扩展原信任。旧 root 保留为来源位置，新位置单独记录；旧事件不重写。0.2.0 本地 SQLite 升至 v3，具备忙检查、一致性备份、事务回滚及回退说明；Web schema 本轮不变。

前三阶段先验证单助手连续工作；L4 单独证明共享和操作系统边界。新设计不废除同 scope 原始历史隔离，只通过明确的长期记忆/历史读取接口共享获准信息。阶段放行、停止条件及 E01–E18 验收矩阵见[开发计划](./local-agent-roadmap.md)。
