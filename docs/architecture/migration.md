...
## 10. 从目录级 CLI 到本机持续记忆（待实施）

新基线为 `d43f07b` / `0.1.7`。当前会话恢复的目录硬检查、cwd 与 grants 共用 root，以及 scope 内的 AgentMemory 适合前期 CLI 验证，但不能直接提供用户要求的跨目录持续记忆。
...
迁移不得把所有任务合成一个 Context scope，不得把旧全部日志自动提升为共享记忆，也不得因移除目录限制而扩展原信任。旧 root 保留为来源位置，新位置单独记录；旧事件不重写。每次实际 schema 变更附顺序迁移、忙检查、一致性备份及回退说明，本文不修改 schema v2。
