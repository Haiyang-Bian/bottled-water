"""Fixed NT namespace operations used by the protected explicit initializer.
...
def open_target(target, *, write=True):
    import win32file

    if target not in TARGETS:
        raise ValueError("Object is outside the fixed namespace dependency list")
    if target.kind == "Device":
        return win32file.CreateFile(target.path, 0x60000 if write else 0x20000, 7, None, 3, 0, None)
    ...
    status = op(ctypes.byref(handle), 0x60000 if write else 0x20000, ctypes.byref(attr))
    if status < 0:
